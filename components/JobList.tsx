...
export default function JobList({
  ...
  const racketTypeOptions = useMemo(() => {
    const relevant =
      racketBrandFilter === ALL ? jobs : jobs.filter((j) => j.racketBrand === racketBrandFilter);
    return [...new Set(relevant.map((j) => j.racketType).filter(Boolean))].sort((a, b) =>
      a.localeCompare(b)
    );
  }, [jobs, racketBrandFilter]);

  // A brand change can leave the current type selection invalid for it
  // (e.g. "Astrox" selected, then brand switched to Victor) — reset rather
  // than silently filtering to zero results with no explanation.
  useEffect(() => {
    if (racketTypeFilter !== ALL && !racketTypeOptions.includes(racketTypeFilter)) {
      setRacketTypeFilter(ALL);
    }
  }, [racketTypeOptions, racketTypeFilter]);
    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-2">
        <input
          ...
          placeholder="Filter by customer name or job ID…"
